    ...
    try {
        ...
        if (!userId || userId === null || !validateIsMongoObjectId(userId) || !username || username === null || !validateEmail(username)) {
            ...
            if(subscribed === 'pending') {
                res.status(400).json('Subscription pending! Please confirm via the link sent to your email!');
            } else 
        ...
        if (
            ...
            if (subscribed === false){
                ...
                if(subStatus !== false && subStatus.$metadata.httpStatusCode === 200) {                    
                    return res.status(200).json('Please check your email for a subscription confirmation link!');
                } else {
